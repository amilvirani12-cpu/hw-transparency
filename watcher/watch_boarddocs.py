...
# meetings this close to today get an auto-generated agenda summary in
# summaries/ — content is deterministic, so the git history records exactly
# when the district published or changed each agenda
SUMMARY_DAYS_PAST = 3
SUMMARY_DAYS_FUTURE = 21
...
def get_agenda_structure(meeting_id):
    """Return the agenda in document order as [('category'|'item', text)]."""
    html = post("BD-GetAgenda", {"id": meeting_id, "current_committee_id": COMMITTEE_ID})
    rows = []
    pat = re.compile(
        r'<span class="category-name">([^<]*)</span>'
        r'|unid="[A-F0-9]{32}"\s+Xtitle="([^"]*)"'
    )
    for m in pat.finditer(html):
        if m.group(1) is not None:
            rows.append(("category", unescape(m.group(1)).strip()))
        else:
            rows.append(("item", unescape(m.group(2)).strip()))
    return rows


def write_meeting_summary(m, date_str, structure, state):
    """Write summaries/<date>_<slug>.md — full agenda + captured documents.

    Content is deterministic (no run timestamps), so re-running with an
    unchanged agenda produces no diff and therefore no commit; every commit
    that does appear timestamps a real change by the district.
    """
    slug = re.sub(r"[^A-Za-z0-9]+", "-", m["name"]).strip("-")[:60] or "meeting"
    dest = SUMMARIES / f"{date_str}_{slug}.md"

    lines = [
        f"# {unescape(m['name']).strip()}",
        "",
        f"**Meeting date:** {date_str}",
        "",
        f"_Agenda as published on the district's [BoardDocs]({BASE}/Public) page. "
        "Auto-generated by the watcher; this file is regenerated whenever the "
        "district updates the agenda, so this repository's commit history records "
        "exactly when each change was published._",
        "",
        "## Agenda",
    ]
    if not any(kind == "item" for kind, _ in structure):
        lines += ["", "_No agenda items posted yet._"]
    else:
        for kind, text in structure:
            if kind == "category":
                lines += ["", f"### {text}", ""]
            else:
                lines.append(f"- {text}")

    files = [f for f in state["files"].values() if f.get("meeting") == m["unique"]]
    lines += ["", "## Financial documents captured by the watcher", ""]
    if files:
        for f in sorted(files, key=lambda x: x["path"]):
            name = f["path"].split("/")[-1]
            cat = f["path"].split("/")[1] if f["path"].count("/") >= 2 else ""
            lines.append(f"- **{cat}** — [{name}](../{quote(f['path'])})")
    else:
        lines.append("_None yet — the watcher checks every morning._")
    lines.append("")
    ...
    if not dest.exists() or dest.read_text(encoding="utf-8") != new:
        ...
        print(f"    summary updated: summaries/{dest.name}")
...
def main():
    ...
    today = datetime.now()
    sum_min = (today - timedelta(days=SUMMARY_DAYS_PAST)).strftime("%Y%m%d")
    sum_max = (today + timedelta(days=SUMMARY_DAYS_FUTURE)).strftime("%Y%m%d")
    ...
    for m in meetings:
        ...
        # agenda summary for meetings near today (e.g. the upcoming board meeting)
        if not dry_run and sum_min <= mdate <= sum_max:
            try:
                structure = get_agenda_structure(mid)
                write_meeting_summary(m, date_str, structure, state)
            except Exception as e:
